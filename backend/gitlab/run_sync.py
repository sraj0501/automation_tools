"""
devtrack gitlab-sync — fetch GitLab issues and report status.

Persists a local snapshot to Data/gitlab/sync_state.json.

Sync modes:
    devtrack gitlab-sync              # Full resync (default)
    devtrack gitlab-sync --full       # Explicit full resync (clears cache)
    devtrack gitlab-sync --hours 24   # Only issues updated in last 24h (merges)

Configure default window via .env:
    GITLAB_SYNC_WINDOW_HOURS=24       # 0 = full sync (default)
"""
...
async def run(full: bool, hours: int | None) -> int:
    ...
    from backend.gitlab.sync import GitLabSync
    ...
    print(f"GitLab Sync  [{client._base_url}]  ({mode_label})")
    print("=" * 55)
    print()

    sync = GitLabSync(client=client)
    ...
    try:
        ...
        if display_items:
            ...
            for issue in display_items:
                ...
                milestone = f"  [{issue.milestone_title}]" if issue.milestone_title else ""
                print(f"  !{issue.iid:<6} {issue.state:<12} {issue.title[:45]}{milestone}{labels}")
            if fetched > 10:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="devtrack gitlab-sync",
        description="Sync GitLab issues to local cache.",
        add_help=False,
    )
